...
usage: Rscript genomic_prediction_stage1_hybrids.R [sim_trait_filename] [outfolder]
...
rm(env, pheno_pop_env, sim_trait_env, predicted_values_env)
...
# order by genotype
results_1st_stage$genotype <- as.character(results_1st_stage$genotype)
results_1st_stage <- results_1st_stage[mixedorder(results_1st_stage$genotype), ]

# remove any genotype with missing data
if (any(is.na(results_1st_stage$predicted_trait_value))) {
  if (!envs_weight) {
    # if no env weights
    results_1st_stage <- pivot_wider(results_1st_stage, names_from = "environment", values_from = "predicted_trait_value")
    results_1st_stage <- na.omit(results_1st_stage)
    results_1st_stage <- pivot_longer(results_1st_stage, -genotype, names_to = "environment", values_to = "predicted_trait_value")
  } else {
    # if env weights
    results_1st_stage <- pivot_wider(results_1st_stage, names_from = "environment", names_sep = "-", values_from = c("predicted_trait_value", "weight"))
    results_1st_stage <- na.omit(results_1st_stage)
    results_1st_stage <- pivot_longer(results_1st_stage, -genotype, names_to = c("type", "environment"), names_sep = "-", values_to = "values")
    results_1st_stage <- pivot_wider(results_1st_stage, names_from = "type", values_from = "values")
  }
}

# save adjusted means for 2nd stage analysis
if (!envs_weight) {
  fwrite(results_1st_stage, file = paste0(outfolder, "/blues_1st_stage.txt"),
         quote = FALSE, sep = "\t", na = NA, row.names = FALSE)
} else {
  fwrite(results_1st_stage, file = paste0(outfolder, "/blues_1st_stage_weighted.txt"),
         quote = FALSE, sep = "\t", na = NA, row.names = FALSE)
}
...
# sim_trait_filename <- "analysis/sim_traits_gwas/EHT/Simulated_Data_3_Reps_Herit_0.488_0.526_0.658_0.494.txt"
# outfolder <- "analysis/sim_traits_gwas/EHT"
# envs_weight <- FALSE
