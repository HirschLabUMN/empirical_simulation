...
usage: Rscript qc_snp-sv_hmp.R [infile] [sv_list] [outfile]
...
# outfolder <- "analysis/qc/snp-sv_hmp"
...
summary_markers[which(!summary_markers[, 1] %in% SVs), "marker_type"] <- "SNP"
summary_markers[which(summary_markers[, 1] %in% SVs), "marker_type"] <- "SV"

# analyze snps and svs separately
for (marker in c("SNP", "SV")) {

  # separate data to have only SNPs or SVs
  summary_markers_type <- subset(summary_markers, marker_type == marker)

  cat("Plotting distribution of", marker, "markers along chromosomes at...\n")
  ...
  for (threshold in c(0.2, 0.3, 0.4, 0.5)) {
    ...
    summary_markers_type$filter <- ifelse(summary_markers_type$missing <= threshold,
                                          yes = "after", no = "before")
    summary_markers_type$filter <- factor(summary_markers_type$filter, levels = c("before", "after"))

    # plot distribution of markers along each chomosome
    dist_chr_plot <- ggplot() +
      geom_density(data = summary_markers_type,
                   mapping = aes(x = pos, fill = "black"),
                   alpha = 0.5, position = "identity") +
      geom_density(data = subset(summary_markers_type, filter == "after"),
                   mapping = aes(x = pos, fill = "firebrick"),
                   alpha = 0.3, position = "identity") +
      facet_wrap(~ chrom, scales = "free_x") +
      labs(subtitle = paste0(marker, "s with <= ", threshold, " missing data"),
           x = "Position (Mb)",
           y = "Density") +
      ...
      theme(text = element_text(size = 20))
    # save plot
    outfile_dist <- paste0(outfolder, "/", marker, "_dist.missing-threshold-", threshold, ".pdf")
    ggsave(filename = outfile_dist, plot = dist_chr_plot, device = "pdf", width = 15, height = 9)

    # write marker names to keep
    outfile_markers <- paste0(outfolder, "/", marker, "s_to_keep.missing-threshold-", threshold, ".txt")
    markers_after_filter <- summary_markers_type[summary_markers_type$filter == "after", 1, drop = FALSE]
