...
usage: Rscript anova_sim_traits_hybrids.R [dir_sim_traits] [...]
...
file_pheno <- args[1]
# file_pheno <- "data/NIFA_CompleteDataset.EHT.txt"
...
pheno_pop <- pheno_pop[mixedorder(pheno_pop$genotype), ]
...
order_geno_env1 <- geno_mean_per_env[which(geno_mean_per_env$environment == geno_mean_per_env$environment[1]), ]
...
pve_fixed <- as.numeric(r.squaredGLMM(trait_lmer)[,"R2m"])
pve_random <- as.numeric(r.squaredGLMM(trait_lmer)[,"R2c"] - pve_fixed)
